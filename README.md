# Maquinas-


##Resumo:
Este projeto tem como principal finalidade a simulação de um site funcional de uma empresa de maquinários. Ele foi estruturado com PHP, MySQL, CSS, JS e HTML, seguindo à risca as orientações do docente.

##Descrição: O sistema oferece uma interface interativa onde os clientes podem consultar o catálogo de máquinas, realizar cadastros, agendamentos e gerenciar pedidos de forma rápida e intuitiva.



##funções:
As principais funções deste projeto são a interação com o banco de dados e as diversas ações, como cadastro, login e também o agendamento de maquinários, deixando a experiência muito mais dinâmica para os usuários


##Com o que foi contruido(tecnologias):

Linguagem:



Guia e explicação:

## Protótipo do Layout (Figma)
Você pode conferir a proposta de design e navegação do projeto pelo link:
->

1. O Modelo de Dados (package model)


   


4. Execução e Testes


Clonar o Repositório

Vídeo (Pitch) do projeto:

Link do vídeo: 
